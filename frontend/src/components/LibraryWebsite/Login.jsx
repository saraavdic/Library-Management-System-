...
const Login = () => {
  ...
  // toastMessage removed per UX preference; keep other states
  useEffect(() => {
    // noop for now; reserved if we need effects later
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store user info in localStorage (exclude password)
      try {
      ...
      // Success message stored (not shown as toast)
      const successMsg = `Welcome back, ${user.first_name || 'User'}!`;
      setSuccess(successMsg);

      // Navigate after a short delay to show success toast
      setTimeout(() => {
        if (user.role === 'admin') {
          // Send admin users to the hidden admin page
          navigate('/admin-6f2b3d');
        } else {
          navigate('/LibraryPortal');
        }
      }, 100);
    } catch (err) {
            ...
            <form onSubmit={handleSubmit} className="login-form">
              ...
              <div className="form-group">
                ...
                <div className="password-wrapper">
                  ...
                  >
                      {/* Use image icons from the public Media folder. */}
                      <img
